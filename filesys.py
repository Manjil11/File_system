...
class File:
    def __init__(self, name, content='', permissions='', parent=None):
        self.name = name
...
class FileSystem:
    ...
    def create_directory(self, name, parent=None):
        ...
        answer = input(f"Do you want to save the directory '{name}' to the system? (y/n): ").lower()
        if answer == 'y':
            # Save the directory to the system
            try:
                os.mkdir(name)
                os.chmod(name, 0o777 & ~self.default_permissions) 
                print("Directory saved to the system.")
            except OSError as e:
                print(f"Error while creating the directory: {e}")
        else:
            print("Directory will not be saved to the system.")

        # Add the created directory to the current working directory in the file system
        self.current_directory.subdirs.append(new_dir)
        return new_dir

    def create_file(self, name, content):
        new_file = File(name, content, self.current_directory)
        answer = input(f"Do you want to save the file '{name}' to the system? (y/n): ").lower()
        if answer == 'y':
            # Save the file to the system
            try:
                with open(name, 'w') as file:
                    file.write(content)
                os.chmod(name, 0o666 & ~self.default_permissions)  
                print("File saved to the system.")
            except OSError as e:
                print(f"Error while saving the file: {e}")
        else:
            print("File will not be saved to the system.")

        # Add the created file to the current working directory in the file system
...
def create_directory(name, permissions, parent):
    new_dir = Directory(name, permissions, parent)
    answer = input(f"Do you want to save the directory '{name}' to the system? (y/n): ").lower()
    if answer == 'y':
        # Save the directory to the system
        try:
    ...
    # Add the created directory to the current working directory in the file system
    fs.current_directory.subdirs.append(new_dir)
    return new_dir

def create_file(name, content, permissions):
    new_file = File(name, content, permissions)
    answer = input(f"Do you want to save the file '{name}' to the system? (y/n): ").lower()
    if answer == 'y':
        # Save the file to the system
        try:
    ...
    # Add the created file to the current working directory in the file system
    fs.current_directory.files.append(new_file)
    return new_file
...
def edit_file_content(file):
    ...
    if permissions_binary & 2:  # Check if the file has write permission
        print(f"Enter new content for file '{file.name}'. Press Ctrl+D to save and exit.")
        new_content = ""
        while True:
            try:
                line = input()
                if len(new_content) + len(line) <= MAX_CONTENT_LENGTH:
                    new_content += " " + line
                else:
                    print(f"File '{file.name}' is full. Cannot add more content.")
            except EOFError:
...
def append_file_content(file):
    permissions_binary = int(str(file.permissions), 8)
    if permissions_binary & 2:  # Check if the file has write permission
        print(f"Enter content to append to file '{file.name}'. Press Ctrl+D to save and exit.")
        while True:
            try:
                append_content = input()
                if len(file.content) + len(append_content) <= MAX_CONTENT_LENGTH:
                    file.content += " " + append_content
                    file.size = len(file.content)
                    file.modified_time = time.time()
                else:
                    print(f"File '{file.name}' is full. Cannot append content.")
            except EOFError:
        ...
        if len(arguments) > 0:
            ...
            if len(arguments) > 1:
                try:
                    ...
                    print("Invalid permissions. Using default permissions (000).")
            add_directory_to_directory(current_directory, create_directory(arguments[0], permissions, current_directory))
        else:
            print("Usage: mkdir <dir_name> [permissions]")
        ...
        if len(arguments) > 0:
            ...
            if len(arguments) > 1:
                try:
                    ...
                    print("Invalid permissions. Using default permissions (000).")
            add_file_to_directory(current_directory, create_file(arguments[0], "", permissions))
        else:
            print("Usage: touch <file_name> [permissions]")
